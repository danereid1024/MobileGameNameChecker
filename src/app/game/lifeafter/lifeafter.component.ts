...
@Component({
  selector: 'app-lifeafter',
  imports: [FormsModule, CommonModule],
  template: `
  <div class="content">
  <div class="title">
  <h1>Life After Name Checker</h1>
  </div>
  ...
  <div class="button">
  <button type="button" class="btn btn-primary" (click)="getLifeAfter()">Check ID</button>
  </div>
  </div>
  `,
  styles: `
...
export class LifeafterComponent {
  ...
  getLifeAfter() {
    ...
    this.nameCheckerService.getLifeAfter(this.id).subscribe({  
      next: (data) => {
        this.nameData = data;
        data.status
        console.log(
