...
@Component({
  selector: 'app-royal-dream',
  imports: [FormsModule, CommonModule],
  template: `
  <div class="content">
  <div class="title">
  <h1>Royal Dream Name Checker</h1>
  </div>
  ...
  <div class="button">
  <button type="button" class="btn btn-primary" (click)="getRoyalDreamID()">Check ID</button>
  </div>
  </div>
  `,
  styles: `
...
export class RoyalDreamComponent {
  ...
  getRoyalDreamID() {
    ...
    this.nameCheckerService.getRoyalDreamID(this.id).subscribe({  
      next: (data) => {
        this.nameData = data;
        data.status
        console.log(
