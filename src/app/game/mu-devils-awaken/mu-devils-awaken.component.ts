...
@Component({
  selector: 'app-mu-devils-awaken',
  imports: [FormsModule, CommonModule],
  template: `
    <div class="content">
  <div class="title">
  <h1>MU: Devil's Awaken Name Checker</h1>
  </div>
  ...
  <div class="button">
  <button type="button" class="btn btn-primary" (click)="getMUDevilsAwaken()">Check ID</button>
  </div>
  </div>
  `,
  styles: `
...
export class MuDevilsAwakenComponent {
  ...
  getMUDevilsAwaken() {
    ...
    this.nameCheckerService.getMUDevilsAwaken(this.id, this.server).subscribe({  
      next: (data) => {
        this.nameData = data;
        data.status
        console.log(
