...
@Component({
  selector: 'app-hay-day',
  imports: [FormsModule, CommonModule],
  template: `
  <div class="content">
  <div class="title">
  <h1>Hay Day Name Checker</h1>
  </div>
  ...
  <div class="input">
    <input  class="text" type="text" [(ngModel)]="tag" name="userId">
  </div>
  <div class="button">
  <button type="button" class="btn btn-primary" (click)="getHayDay()">Check ID</button>
  </div>
  </div>
  `,
   styles: ` 
...
export class HayDayComponent {
  ...
  getHayDay() {
    if (this.tag.trim() === '') {
      alert("Please insert user ID and server tag!")
    }

    this.nameCheckerService.getHayDay(this.tag).subscribe({  
      next: (data) => {
        this.nameData = data;
        data.status
        console.log(
