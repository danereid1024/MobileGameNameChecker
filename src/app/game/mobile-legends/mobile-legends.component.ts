...
@Component({
  selector: 'app-mobile-legends',
  imports: [FormsModule, CommonModule],
  template: `
  <div class="content">
  <div class="title">
  <h1>Mobile Legends Name Checker</h1>
  </div>
...
export class MobileLegendsComponent {
  ...
  getMobileLegendsID() {
    if (this.id.trim() === '' || this.server.trim() == '') {
      alert("Please insert user ID and server ID!")
    }

    this.nameCheckerService.getMobileLegendsID(this.id, this.server).subscribe({  
      next: (data) => {
        this.nameData = data;
        data.status
        console.log(
