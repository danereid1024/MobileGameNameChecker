...
@Component({
  ...
  <button type="button" class="btn btn-primary" (click)="getClashRoyale()">Check Tag</button>
  </div>
  </div>
  `,
  styles: `
...
export class ClashRoyaleComponent {
  ...
  getClashRoyale() {
    ...
    this.nameCheckerService.getClashRoyale(this.tag).subscribe({  
      next: (data) => {
        this.nameData = data;
        data.status
        console.log(
